    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        match action.as_str() {
            "create" => {
                ...
                let amount_sompi = try_parse_required_nonzero_vecno_as_sompi_u64(argv.get(1))?;
                let outputs = PaymentOutputs::from((address, amount_sompi));
                let priority_fee_sompi = try_parse_optional_vecno_as_sompi_i64(argv.get(2))?.unwrap_or(0);
            ...
            "script" => {
                ...
                match subcommand.as_str() {
                    "lock" => {
                        let amount_sompi = try_parse_required_nonzero_vecno_as_sompi_u64(argv.first())?;
                        let outputs = PaymentOutputs::from((script_p2sh, amount_sompi));
                        let priority_fee_sompi = try_parse_optional_vecno_as_sompi_i64(argv.get(1))?.unwrap_or(0);
                        let abortable = Abortable::default();
                    ...
                    "unlock" => {
                        ...
                        let priority_fee_sompi = try_parse_optional_vecno_as_sompi_i64(argv.first())?.unwrap_or(0) as u64;
                        ...
                        tprintln!(
                            ...
                            sompi_to_vecno(total_locked_sompi)
                        );
            ...
            "parse" => {
                ...
                let pskb = Self::parse_input_pskb(argv.first().unwrap().as_str())?;
                tprintln!(ctx, "{}", pskb.display_format(ctx.wallet().network_id()?, sompi_to_vecno_string_with_suffix));
