...
use vecno_utils::hex::{FromHex, ToHex};
...
impl PoW {
    ...
    pub fn new(header: &HeaderT, timestamp: Option<u64>) -> Result<PoW> {
        ...
        // Calculate pre_pow_hash by zeroing out nonce and time
        let pre_pow_hash = hashing::header::hash_override_nonce_time(header, 0, 0);
        // Use the provided timestamp or fallback to header timestamp
        ...
        Ok(Self {
            inner: State {
                target,
                hasher,
                timestamp: timestamp.unwrap_or(header.timestamp),
            },
            pre_pow_hash,
        })
    }
...
/// Calculates target from difficulty, based on set_difficulty function
/// @category Mining
