...
use vecno_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl, SessionLock};
use vecno_core::{debug, time::unix_now, warn};
use vecno_database::{
    prelude::{
...
impl ConsensusFactory for Factory {
    fn new_active_consensus(&self) -> (ConsensusInstance, DynConsensusCtl) {
        ...
        let db = vecno_database::prelude::ConnBuilder::default()
            .with_db_path(dir)
