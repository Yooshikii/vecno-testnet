...
use vecno_consensus_core::hashing::sighash::{calc_schnorr_signature_hash, SigHashReusedValues};
use vecno_consensus_core::hashing::sighash_type::SIG_HASH_ALL;
use vecno_consensus_core::tx;

pub fn script_hashes(mut mutable_tx: tx::SignableTransaction) -> Result<Vec<vecno_hashes::Hash>> {
    let mut list = vec![];
