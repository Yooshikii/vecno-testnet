...
impl TryFrom<VecnodConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: VecnodConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no vecnod path is specified".to_string()));
        }
...
impl Vecnod {
    pub fn new(args: VecnodConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: VecnodConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Vecno node is already running.".to_string()));
            }
        }
        ...
        // let options = VecnodOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Vecnod binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}
...
pub trait VecnodCtl {
    ...
    async fn configure(&self, config: VecnodConfig) -> Result<()>;
