...
use vecno_rpc_core::{api::rpc::RpcApi, notify::mode::NotificationMode};
...
impl Args {
    fn parse() -> Self {
        ...
        Args {
            ...
            rpc_server: m.get_one::<String>("rpcserver").cloned().unwrap_or("localhost:7110".to_owned()),
...
async fn main() {
    ...
    info!("Using Rothschild with private key {} and address {}", schnorr_key.display_secret(), String::from(&vecno_addr));
    ...
    let txs = selected_utxos_groups
        ...
        .map(|utxo_option| {
            if let Some((selected_utxos, selected_amount)) = utxo_option {
                let tx = generate_tx(schnorr_key, &selected_utxos, selected_amount, num_outs, &vecno_addr);
