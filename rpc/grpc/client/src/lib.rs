...
use vecno_utils::{channel::Channel, triggers::DuplexTrigger};
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(VecnodPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type VecnodRequestSender = async_channel::Sender<VecnodRequest>;
type VecnodRequestReceiver = async_channel::Receiver<VecnodRequest>;
...
/// Data flow:
/// ```
/// //   VecnodRequest -> request_send -> stream -> VecnodResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: VecnodRequestSender,
    request_receiver: VecnodRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<VecnodResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual VecnodRequest to VecnodResponse stream
        let mut stream: Streaming<VecnodResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: VecnodPayloadOps, request: impl Into<VecnodRequest>) -> Result<VecnodResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: VecnodRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<VecnodResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = vecnod_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = vecnod_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
