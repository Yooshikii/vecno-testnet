    ...
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: vecno_rpc_core::Notification| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: Vecno RPC notification is not consumed by user: {:?}", notification);
                        }
                        Ok(())
                    })
                }),
...
/// [`VecnoRpcClient`] allows connection to the Vecno wRPC Server via
...
impl VecnoRpcClient {
    /// Create a new `VecnoRpcClient` with the given Encoding and URL
    pub fn new(
        encoding: Encoding,
        url: Option<&str>,
