...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to Vecno node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // If you want to connect to your own node, define your node address and wRPC port using let url = Some("ws://0.0.0.0:8110")
    // Verify your Vecno node is runnning with --rpclisten-borsh=0.0.0.0:8110 parameter
    ...
    // Define the network your Vecno node is connected to
    ...
    // Connect to selected Vecno node
    client.connect(Some(options)).await?;

    // Retrieve and show Vecno node information
    ...
    // Disconnect client from Vecno node
