// Basic example of a Vecno wRPC client that connects to a node
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Vecno RPC client instance using Borsh encoding.
        let client = Arc::new(VecnoRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
