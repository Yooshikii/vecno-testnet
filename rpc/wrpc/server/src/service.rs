...
impl Default for Options {
    fn default() -> Self {
        Options { listen_address: "127.0.0.1:7110".to_owned(), verbose: false, grpc_proxy_address: None }
    }
}

/// ### VecnoRpcHandler
///
/// [`VecnoRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct VecnoRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl VecnoRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        VecnoRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for VecnoRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "vecno" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<VecnoRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(VecnoRpcHandler::new(tasks, *encoding, core_service, options.clone()));
