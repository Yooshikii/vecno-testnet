//!
//! # WASM bindings for the [Vecno p2p Node RPC client](VecnoRpcClient).
//!
//! This module provides a WASM interface for the Vecno p2p Node RPC client
//! - [`RpcClient`].
//!
...
use vecno_notify::connection::ChannelType;
use vecno_notify::events::EventType;
use vecno_notify::listener;
use vecno_notify::notification::Notification as NotificationT;
use vecno_rpc_core::api::ctl;
pub use vecno_rpc_core::wasm::message::*;
pub use vecno_rpc_macros::{
...
pub(crate) struct Inner {
    ...
    notification_channel: Channel<vecno_rpc_core::Notification>,
}
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            VecnoRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<VecnoRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<VecnoRpcClient> {
        &self.inner.client
    }
                ...
                select_biased! {
                    ...
                    msg = notification_receiver.recv().fuse() => {
                        if let Ok(notification) = &msg {
                            match &notification {
                                vecno_rpc_core::Notification::UtxosChanged(utxos_changed_notification) => {
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = VecnoRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
