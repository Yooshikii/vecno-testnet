...
use vecno_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  vecno_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut vecnod1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = vecnod1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut vecnod2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = vecnod2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut vecnod1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut vecnod2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = vecnod1.start().await;
    let rpc_client2 = vecnod2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", vecnod1.p2p_port).try_into().unwrap(), true).await.unwrap();
...
/// `cargo test --release --package vecno-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("vecno-testing-integration-heap.json").build();

    vecno_core::log::try_init_logger(
        "INFO,vecno_testing_integration=trace,vecno_notify=debug,vecno_rpc_core=debug,vecno_grpc_client=debug",
    );
    ...
    let miner_address =
        Address::new(vecnod1.network.into(), vecno_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(vecnod1.network.into(), vecno_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(vecnod1.network.into(), vecno_addresses::Version::PubKey, &[0; 32]);
    ...
    let mut clients = vec![ListeningClient::connect(&vecnod2).await, ListeningClient::connect(&vecnod1).await];
    ...
    assert_eq!(miner_balance, initial_blocks * SIMNET_PARAMS.premine_phase_base_subsidy);
    ...
    for utxo in utxos.iter() {
        ...
        assert_eq!(utxo.1.amount, SIMNET_PARAMS.premine_phase_base_subsidy);
    ...
    const TX_AMOUNT: u64 = SIMNET_PARAMS.premine_phase_base_subsidy * (NUMBER_INPUTS * 5 - 1) / 5;
    ...
    for x in clients.iter() {
        ...
        assert_eq!(
            ...
            SIMNET_PARAMS.premine_phase_base_subsidy * NUMBER_INPUTS
        );
    ...
    for x in clients.iter() {
        ...
        assert_eq!(miner_balance, (initial_blocks - NUMBER_INPUTS) * SIMNET_PARAMS.premine_phase_base_subsidy);
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    vecno_core::log::try_init_logger("info,vecno_grpc_core=trace,vecno_grpc_server=trace,vecno_grpc_client=trace,vecno_core=trace");
    ...
    {
        let total_fd_limit = 10;
        let mut vecnod1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = vecnod1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = vecnod1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&vecnod1.core);
