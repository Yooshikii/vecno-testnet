...
use vecno_consensus_core::{constants::MAX_SOMPI, header::Header, subnets::SubnetworkId, tx::Transaction};
...
async fn sanity_test() {
    ...
    for op in VecnodPayloadOps::iter() {
        ...
        let task: JoinHandle<()> = match op {
            ...
            VecnodPayloadOps::GetCurrentBlockColor => {
                tst!(op, "see SubmitBlock")
            }
            ...
            VecnodPayloadOps::SubmitTransactionReplacement => {
                let rpc_client = client.clone();
                tst!(op, {
                    // Build an erroneous transaction...
                    let transaction = Transaction::new(0, vec![], vec![], 0, SubnetworkId::default(), 0, vec![]);
            ...
            VecnodPayloadOps::GetConnections => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VecnodPayloadOps::GetSystemInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VecnodPayloadOps::GetFeeEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            VecnodPayloadOps::GetFeeEstimateExperimental => {
                let rpc_client = client.clone();
                tst!(op, {
