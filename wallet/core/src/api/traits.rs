//!
//! API trait for interfacing with the Vecno wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `vecno`.
    ///
    ...
    /// applications such as KDX and vecno web wallet using a deprecated derivation path
