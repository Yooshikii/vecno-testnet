...
// ---
// {"type":"vecno-wallet","encryption":"default","version":1,"generator":"pwa","wallet":{"mnemonic":"hex"}}
...
fn legacy_v0_keydata_location() -> Result<(PathBuf, Options)> {
    let filename = if runtime::is_windows() {
        ...
        fs::resolve_path(&format!("{appdata}/Vecno/vecno.kpk"))?
    } else if runtime::is_macos() {
        fs::resolve_path("~/Library/Application Support/Vecno/vecno.kpk")?
    } else {
        fs::resolve_path("~/.vecno/vecno.kpk")?
    };

    let options = workflow_store::fs::Options::with_local_storage_key("vecno-wallet");
