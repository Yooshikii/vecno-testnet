...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] vecno_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    VecnoRpcClientResult(#[from] VecnoRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    VecnoWorkflowRpcError(#[from] VecnoWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] vecno_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] vecno_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] vecno_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] vecno_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] vecno_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] vecno_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] vecno_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] vecno_txscript_errors::TxScriptError),
