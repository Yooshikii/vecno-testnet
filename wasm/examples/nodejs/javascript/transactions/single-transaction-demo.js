...
(async () => {
    ...
    // An address such as vecno:qr0lr4ml9fn3chekrqmjdkergxl93l4wrk3dankcgvjq776s9wn9jkdskewva
    const address = keypair.toAddress(networkId);
    console.info(`Full vecno address: ${address}`);
    ...
    try {
        ...
        if (utxos.length === 0) {
            console.info('Send some vecno to', address, 'before proceeding with the demo');
            return;
        }
