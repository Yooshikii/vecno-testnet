    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToVecnoString(value(tx)||0)
            });
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        list.push({
                            ...
                            Mature: sompiToVecnoString(data.balance.mature),
                            Pending: sompiToVecnoString(data.balance.pending),
                            Outgoing: sompiToVecnoString(data.balance.outgoing),
